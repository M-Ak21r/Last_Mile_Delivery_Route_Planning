...
import { badge, formatDate, loading, showToast, confirm, openModal, closeModal } from '../utils/ui.js';
import type { Delivery, Priority, DeliveryStatus } from '../types/index.js';
...
function bindDeliveryEvents(container: HTMLElement): void {
  ...
  container.querySelector('#status-filters')?.addEventListener('click', (e) => {
    ...
    else delete currentFilter.status;
    loadDeliveries();
  });

  container.querySelector('#priority-filter')?.addEventListener('change', (e) => {
    const val = (e.target as HTMLSelectElement).value;
    if (val) currentFilter.priority = val;
    else delete currentFilter.priority;
    loadDeliveries();
  });
  ...
  container.addEventListener('click', async (e) => {
    const editBtn = (e.target as HTMLElement).closest('.edit-btn') as HTMLElement;
    const deleteBtn = (e.target as HTMLElement).closest('.delete-btn') as HTMLElement;
    if (editBtn) {
...
function openDeliveryForm(delivery?: Delivery): void {
      ...
      <div class="form-group">
        ...
        <select class="form-select" id="f-priority">
          ${(['low','medium','high','urgent'] as Priority[]).map(p =>
            `<option value="${p}" ${delivery?.priority === p ? 'selected' : ''}>${p}</option>`).join('')}
        </select>
      </div>
      <div class="form-group">
  ...
  document.getElementById('save-delivery-btn')?.addEventListener('click', async () => {
    const body = {
      ...
      priority:        (document.getElementById('f-priority') as HTMLSelectElement).value as Priority,
      status:          (document.getElementById('f-status') as HTMLSelectElement).value as DeliveryStatus,
      deliveryWindow: {
        start: (document.getElementById('f-wstart') as HTMLInputElement).value,
        end:   (document.getElementById('f-wend') as HTMLInputElement).value,
      },
      specialInstructions: (document.getElementById('f-notes') as HTMLTextAreaElement).value,
    };
