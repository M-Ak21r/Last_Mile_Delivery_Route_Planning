...
import { badge, vehicleBadge, driverAvatar, showToast, confirm, openModal, closeModal } from '../utils/ui.js';
let allDrivers = [];
...
async function loadDrivers(statusFilter) {
...
function renderDriverCards(drivers) {
    ...
    const vehicleIcons = { bike: '🚲', scooter: '🛵', van: '🚐', truck: '🚛' };
...
function bindDriverEvents(container) {
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <select class="form-select" id="df-vtype">
          ${['bike', 'scooter', 'van', 'truck'].map(v => `<option value="${v}" ${driver?.vehicleType === v ? 'selected' : ''}>${v}</option>`).join('')}
        </select>
      </div>
      <div class="form-group">
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        ...
        <select class="form-select" id="df-status">
          ${['available', 'on_route', 'off_duty', 'break'].map(s => `<option value="${s}" ${driver?.status === s ? 'selected' : ''}>${s.replace('_', ' ')}</option>`).join('')}
        </select>
      </div>
    </div>
  ...
  `;
    ...
    document.getElementById('save-driver-btn')?.addEventListener('click', async () => {
        const body = {
            ...
            vehicleType: document.getElementById('df-vtype').value,
            vehicleNumber: document.getElementById('df-vnum').value,
            capacityKg: parseFloat(document.getElementById('df-cap').value),
            status: document.getElementById('df-status').value,
        };
        try {
            if (isEdit) {
                await updateDriver(driver._id, body);
