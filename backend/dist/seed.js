...
const DEPOT_COORDS = [77.209, 28.6139];
...
async function seed() {
    try {
        ...
        // Clear existing data
        await Promise.all([Driver_1.default.deleteMany({}), Delivery_1.default.deleteMany({}), Route_1.default.deleteMany({})]);
        console.log('Cleared existing data');
        // Insert drivers
        const drivers = await Driver_1.default.insertMany(driversData);
        ...
        const deliveriesWithIds = deliveriesData.map((d) => ({
            ...d,
            orderId: `ORD-${(0, uuid_1.v4)().slice(0, 8).toUpperCase()}`,
        }));
        const deliveries = await Delivery_1.default.insertMany(deliveriesWithIds);
